...
def scale_time(gpx,cyc_time):
    scale_t = timedelta(hours=11, minutes=30)/cyc_time

    cyc_time = timedelta(seconds=0)
    time_s=last_t = gpx.tracks[0].segments[0].points[0].time
    for track in gpx.tracks:
        for segment in track.segments:
            for point in segment.points:
                ...
                cur_t = point.time
                if (cur_t-last_t <= timedelta(seconds=restTime)):
...
def procXml(gpxPath, time_new):
    ...
    while (cyc_time > timedelta(hours=12)):
        gpx,cyc_time=scale_time(gpx,cyc_time)
...
def main():
    ...
    if args.d:
        ...
        if len(fileList)<1:
            print('the gpx file not found, please check!')
